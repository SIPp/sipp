...
static int rtpstream_start_task (rtpstream_callinfo_t *callinfo)
{
    ...
    if (ready_index==num_ready_threads) {
        ...
        printAudioHex("CREATED THREAD: ", "", 0, threadID, 0);
        printVideoHex("CREATED THREAD: ", "", 0, threadID, 0);
...
int rtpstream_shutdown(std::tr1::unordered_map<pthread_t, std::string>& threadIDs)
{
    ...
    {
        printAudioHex("EXISTING THREADID: ", "", 0, iter->first, 0);
        printVideoHex("EXISTING THREADID: ", "", 0, iter->first, 0);
